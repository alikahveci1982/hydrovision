...
if final_file:
    ...
    if st.button(L["analyze_button"], type="primary"):
        try:
            ...
            st.markdown(f'<a href="https://www.google.com/search?q={query}" target="_blank" style="text-decoration:none;"><div style="width:100%; background:linear-gradient(135deg, var(--accent-cyan), var(--accent-blue)); color:var(--bg-base); text-align:center; border-radius:12px; padding:16px; font-family:Rajdhani,sans-serif; font-weight:700; font-size:15px; letter-spacing:2px; margin-top:12px; box-shadow: 0 6px 28px rgba(0,224,245,0.35);"> {L["purchase_button"]} </div></a>', unsafe_allow_html=True)
